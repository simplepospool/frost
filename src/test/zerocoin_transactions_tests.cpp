...
BOOST_AUTO_TEST_CASE(zerocoin_spend_test)
{
    ...
    cWallet.zfrostTracker = unique_ptr<CzFROSTTracker>(new CzFROSTTracker(cWallet.strWalletFile));
    ...
    BOOST_CHECK_MESSAGE(receipt.GetStatus() == ZFROST_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
    ...
    BOOST_CHECK_MESSAGE(receipt2.GetStatus() == ZFROST_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
