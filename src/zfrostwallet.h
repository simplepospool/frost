...
#ifndef BIFROST_ZFROSTWALLET_H
#define BIFROST_ZFROSTWALLET_H
...
public:
    CzFROSTWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZFROST(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZFROST(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //BIFROST_ZFROSTWALLET_H
