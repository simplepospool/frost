...
int64_t GetBlockValue(int nHeight)
{
    ...
    } else if (nHeight < Params().WalletForkBlock() && nHeight > 200) {
        nSubsidy = 50 * COIN;       // Year 1
    } else if (nHeight <= Params().WalletForkDblBlock() && nHeight >= Params().WalletForkBlock()) {
        nSubsidy = 1 * COIN;        // transition
    } else if (nHeight <= Params().WalletForkResumeBlock() && nHeight > Params().WalletForkDblBlock()) {
        nSubsidy = 100 * COIN;      // Double Rewards
    } else if (nHeight <= 788400 && nHeight > Params().WalletForkResumeBlock()) {
        nSubsidy = 10 * COIN;       // Year 2
    } else if (nHeight <= 1576800 && nHeight > 788400) {
        nSubsidy = 9 * COIN;        // Year 3
    } else if (nHeight <= 2102400 && nHeight > 1576800) {
        nSubsidy = 8 * COIN;       // Year 4
    } else if (nHeight <= 2628000 && nHeight > 2102400) {
        nSubsidy = 6.5 * COIN;      // Year 5
    } else if (nHeight <= 3153600 && nHeight > 2628000) {
        nSubsidy = 4 * COIN;        // Year 6
    } else if (nHeight <= 3679200 && nHeight > 3153600) {
        nSubsidy = 2.5 * COIN;      // Year 7
    } else if (nHeight <= 4204800 && nHeight > 3679200) {
        nSubsidy = 1.4 * COIN;      // Year 8
    } else if (nHeight <= 4730400 && nHeight > 4204800) {
        nSubsidy = 0.8 * COIN;      // Year 9
    } else if (nHeight <= 5256000 && nHeight > 4730400) {
        nSubsidy = 0.5 * COIN;      // Year 10
    } else if (nHeight > 5256000) {
        nSubsidy = .001 * COIN;     //  Beyond year 10
    } else {
        nSubsidy = 2 * COIN;
    }
    return nSubsidy;
...
bool IsMasternodeCollateral(CAmount value)
{
    if (chainActive.Height() < Params().WalletForkBlock()) {
        ...
        if (value == MASTERNODE_COLLATERAL_OLD && !IsSporkActive(SPORK_17_MASTERNODE_COLLATERAL_ENFORCEMENT)) {
            return true;
        } else if (value == MASTERNODE_COLLATERAL_NEW) {
            return true;
        } else {
            return false;
        }
...
//              FormatMoney(nFees), FormatMoney(pindex->nMint), FormatMoney(nAmountZerocoinSpent));
    ...
    if (!IsBlockValueValid(block, nExpectedMint, pindex->nMint)) {
        if (chainActive.Height() >= Params().WalletForkBlock()) {
            ...
            LogPrint("blockvalue", "ConnectBlock() : reward pays too much (actual=%s vs limit=%s) - ignored",
                    FormatMoney(pindex->nMint), FormatMoney(nExpectedMint));
        }
    }
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, CBlockIndex* const pindexPrev)
{
    ...
    // Don't check the prefork blocks (versions are messed up and bug in the miner)
    if (nHeight < Params().WalletForkBlock() + Params().ToCheckBlockUpgradeMajority()) {
        return true;
    }
