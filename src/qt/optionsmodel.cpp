...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::FROST);
    ...
    if (!settings.contains("nAnonymizeBifrostAmount"))
        settings.setValue("nAnonymizeBifrostAmount", 1000);

    nAnonymizeBifrostAmount = settings.value("nAnonymizeBifrostAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeBifrostAmount"))
        SoftSetArg("-anonymizebifrostamount", settings.value("nAnonymizeBifrostAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in bifrost.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeBifrostAmount:
            return QVariant(nAnonymizeBifrostAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeBifrostAmount:
            nAnonymizeBifrostAmount = value.toInt();
            settings.setValue("nAnonymizeBifrostAmount", nAnonymizeBifrostAmount);
            emit anonymizeBifrostAmountChanged(nAnonymizeBifrostAmount);
            break;
        case CoinControlFeatures:
